...
Note the cache is off by default and only four places switch it on: dashboard during a run, both tax tools, and the toolkit shell while you're on `#home` (which covers tools hosted inside it). A tool opened standalone runs with in-flight dedupe only. That makes `fresh: true` a no-op wherever the cache is off — it's a correctness marker for calls that must never serve a warmed value (donation-draw resolves money), not something that changes behaviour today.
